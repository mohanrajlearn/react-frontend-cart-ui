  ...
  performAPICall = async () => {
    ...
    try {
     // response = await (await fetch(`${config.endpoint}/products`)).json();
      response = await getApiResponse(`${config.endpoint}/products`,"GET");
    } catch (e) {
      errored = true;
    }
      ...
      <>
        ...
        <Row>
              ...
              <Row>
                {this.products.length ? (
                  this.state.filteredProducts.map((product) =>(
                    <Col xs={12} sm={12} lg={6} key={product._id}>
                      <Product
                        product={product}
                        addToCart={() => {
                          if (this.state.loggedIn) {
                            this.cartRef.current.pushToCart(
                              product._id,
                              1,
                              true
                            );
         ...
         {this.state.loggedIn && this.products.length && (
            <Col xs={{ span: 24 }} md={6} className="search-cart">
              <div>
                <Cart
                  ref={this.cartRef}
