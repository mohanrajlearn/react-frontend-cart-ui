...
getApiResponse = async (url,method,headers,body) => {
let response = await (
        await fetch(url, {
          method: method,
          headers: headers,
...
this.setState({
  ...
  performAPICall = async () => {
    ...
    // 1.show spinner on the register button
    this.showSpinner();

    // 2.Getting the API response
    try {
      response = this.getApiResponse(`${config.endpoint}/auth/register`,"POST",
      {
    ...
    // 3.hide spinner shown on the register button
